...
@section('pages-content')
    <main id="js-page-content" role="main" class="page-content">
        ...
        <form action="{{ route('outcoming.update', $outcoming->id) }}" method="POST">
            @csrf
            @method('PUT')
            <x-panel.show title="Edit" subtitle="Barang Keluar">
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input type="text" class="form-control" name="tanggal" id="tanggal"
                            value="{{ old('tanggal', $outcoming->purchase_date) }}" readonly placeholder="Select date" id="datepicker-2">
                        @error('tanggal')
                ...
                <div class="form-group">
                    <label for="kode_masuk">Kode Barang Keluar</label>
                    <input type="text" readonly name="kode_keluar" id="kode_keluar" class="form-control"
                        value="{{ old('kode_keluar', $outcoming->kode) }}">
                    @error('kode_keluar')
                        <span class="text-danger help-block">{{ $message }}</span>
                    @enderror
                </div>              
                <div class="form-group">
                    ...
                    <select class="select2 form-control @error('id_product') is-invalid @enderror" aria-label="id_product"
                        ...
                        @foreach ($products as $cat)
                            <option value="{{ $cat->id }}" {{ old('id_product', $outcoming->product->id ?? '') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}</option>
                        @endforeach
                    </select>
                    @error('id_product')
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="quantity" id="quantity" class="form-control" value="{{ old('quantity', $outcoming->quantity) }}"
                        required>
                    @error('quantity')
                        <span class="text-danger help-block">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="unit" id="unit" class="form-control" value="{{ old('unit', $outcoming->unit) }}">
                    @error('unit')
                        <span class="text-danger help-block">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" name="bagian" id="bagian" class="form-control"
                        value="{{ old('bagian', $outcoming->bagian) }}">
                    @error('bagian')
                        <span class="text-danger help-block">{{ $message }}</span>
                    @enderror
                </div>
                <div class="form-group">
                    ...
                    <input type="text" name="no_spk" id="no_spk" class="form-control"
                        value="{{ old('no_spk', $outcoming->no_spk) }}">
                    @error('no_spk')
                        <span class="text-danger help-block">{{ $message }}</span>
                    @enderror
                </div>
...
{{-- @section('pages-script')
    ...
    <script>
        // Generate the outcoming code when the page loads
