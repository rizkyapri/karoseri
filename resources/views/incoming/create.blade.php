...
@section('pages-content')
    <main id="js-page-content" role="main" class="page-content">
        ...
        <form action="{{ route('incoming.store') }}" method="POST">
            @csrf
            <x-panel.show title="Tambah" subtitle="Barang Masuk">
                ...
                <div class="form-group">
                    ...
                    <select class="select2 form-control w-100 @error('id_supplier') is-invalid @enderror" aria-label="id_supplier"
                        id="id_supplier" name="id_supplier">
                        <option selected disabled>- Pilih Produk -</option>
                        @foreach ($suppliers as $cat)
                            <option value="{{ $cat->id }}" {{ old('id_supplier') == $cat->id ? 'selected' : '' }}>
                                {{ $cat->name }}</option>
                        @endforeach
                    </select>
                    @error('id_supplier')
